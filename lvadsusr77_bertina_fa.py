...
print(sales_trend)
print("from the sales trend analysis we can infer that every year the sales amount is increasing\n")

data['Order Date'] = pd.to_datetime(data['Order Date'])
data['Year'] = data['Order Date'].dt.year
category_sales = data.groupby(['Year', 'Category'])['Sales'].sum().unstack()
sales_growth = category_sales.pct_change(axis=0) * 100
total_growth = sales_growth.sum()
most_growth = total_growth.idxmax()
print("Category with the most growth in terms of sales over the years:", most_growth)
